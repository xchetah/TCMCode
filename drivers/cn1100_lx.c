...
#include "../../../arch/arm/mach-lc181x/board/board-lc1913_v3_ws_l706.h"
#include <linux/seq_file.h>

int SCREEN_HIGH=800;
int SCREEN_WIDTH=480;
int CN1100_RESET_PIN=TC1126_RST_PIN;
int CN1100_INT_PIN=TC1126_INT_PIN;
...
void DataProc_FaceDetectProcess(void)
{
    ...
    if(1 == tpd_proximity_flag){
        ...
        if(FACE_DETECT_NEAR == bdt.FDC.Flag){
            tpd_proximity_detect = 1;
        }else{
            tpd_proximity_detect = 0;
        }
        if(last_tpd_proximity_state != tpd_proximity_detect){
            ws_datap->prox_event = 1;
            wake_up_interruptible(&ws_datap->proximity_event_wait);
        }
        last_tpd_proximity_state = tpd_proximity_detect;
    }
}
...
static ssize_t prox_read(struct file *file,char *buf, size_t count, loff_t *ppos)
{
	...
	if (!data->prox_event|| !buf)
		return 0;
	data->prox_state = tpd_proximity_detect; //jacky
...
static int chm_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(!spidev->chm_ts_proc){
        printk("Error to create debug entry\n");
    }
#endif
...
static int __init cn1100_spi_init(void)
{
	...
	client = i2c_new_device(adapter,&i2c_info);
	if(!client){
		return -1;
	}
