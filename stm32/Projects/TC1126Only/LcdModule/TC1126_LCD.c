...
#ifdef LCD_800X480_2PixelsOnlyWorking
...
#ifdef LCD_800X480_1PixelsOnlyWorking
...
#ifdef LCD_320X240_ILI9328
void LCD_Init_ILI9325_320_240(void)
{
...
//	LCD_WR(0x0000);
//	device_id=LCD_RD();	
//	CN1100_print("%4x",device_id);

        LCD_ID(0x0001,0x0100);     
        LCD_ID(0x0002,0x0700);//电源开启                    
				//LCD_ID(0x0003,(1<<3)|(1<<4) ); 	//65K  RGB
				//DRIVE TABLE(寄存器 03H)
        ...
        LCD_ID(0x0003,0x1020|(1<<3));//65K    
        LCD_ID(0x0004,0x0000);                                   
        LCD_ID(0x0008,0x0202);	           
        LCD_ID(0x0009,0x0000);         
        LCD_ID(0x000a,0x0000);//display setting         
        LCD_ID(0x000c,0x0000);//display setting          
        LCD_ID(0x000d,0x0000);//0f3c          
        LCD_ID(0x000f,0x0000);
		//电源配置
        LCD_ID(0x0010,0x0000);   
        LCD_ID(0x0011,0x0007);
        LCD_ID(0x0012,0x0000);                                                                 
        LCD_ID(0x0013,0x0000);           
        LCD_ID(0x0007,0x0001);       
        Stupid_Delay(5); 
        LCD_ID(0x0010,0x1690);   
        LCD_ID(0x0011,0x0227);
        Stupid_Delay(5); 
        LCD_ID(0x0012,0x009c);                  
        Stupid_Delay(5); 
        LCD_ID(0x0013,0x1800);   
        LCD_ID(0x0029,0x0021);
				LCD_ID(0x002b,0x000d);
        Stupid_Delay(5); 
        LCD_ID(0x0020,0x0000);
				LCD_ID(0x0021,0x0000);        
		//伽马校正
        LCD_ID(0x0030,0x0000); 
        LCD_ID(0x0031,0x0507);   
        LCD_ID(0x0032,0x0004);
        LCD_ID(0x0035,0x0205);
        LCD_ID(0x0036,0x0004); 
        LCD_ID(0x0037,0x0307);
        LCD_ID(0x0038,0x0002);        
        LCD_ID(0x0039,0x0707);     
        LCD_ID(0x003c,0x0502);
        LCD_ID(0x003d,0x0004);
 
        LCD_ID(0x0050,0x0000); //水平GRAM起始位置 
        LCD_ID(0x0051,0x00ef); //水平GRAM终止位置                    
        LCD_ID(0x0052,0x0000); //垂直GRAM起始位置                    
        LCD_ID(0x0053,0x013f); //垂直GRAM终止位置  
        
        LCD_ID(0x0060,0xa700);        
        LCD_ID(0x0061,0x0001); 
        LCD_ID(0x006a,0x0000);
        LCD_ID(0x0080,0x0000);
        LCD_ID(0x0081,0x0000);
        LCD_ID(0x0082,0x0000);
        LCD_ID(0x0083,0x0000);
        LCD_ID(0x0084,0x0000);
        LCD_ID(0x0085,0x0000);
      
        LCD_ID(0x0090,0x0010);     
        LCD_ID(0x0092,0x0600);

        //开启显示设置    
        LCD_ID(0x0020,0x0000);
				LCD_ID(0x0021,0x0000);   
		
        LCD_ID(0x0007,0x0133);   

}

#endif  //LCD_320X240_ILI9328
...
void LCD_fill(uint16_t color)
{
  ...
  SetXYStartAddrEndAddr(0, LCD_SCREEN_HIGH - 1, 0, LCD_SCREEN_WIDTH - 1);
  while(n<(LCD_SCREEN_HIGH*LCD_SCREEN_WIDTH))   // 全屏填充   
  {
      LCD_WD(color);
      n++;
  }   
...
void LCD_PointFill(uint16_t x1, uint16_t y1, uint16_t color)
{
  ...
  #ifdef LCD_320X240_ILI9328
    SetXYStartAddrEndAddr(x1, x1, y1, y1);
    LCD_WD(color);
  #endif
  #ifdef LCD_800X480_1PixelsOnlyWorking
    SetXYStartAddrEndAddr(x1, x1, y1, y1);
...
void TFT_ShowBar(uint16_t x, uint16_t y, uint16_t bar, uint16_t Scolor)
{
    uint16_t i, x0 = x<<3, x1;
    uint16_t y0 = 4 + (y<<4);

    x1 = x0 + bar;
    if(x1 > LCD_SCREEN_HIGH) x1 = LCD_SCREEN_HIGH;
    SetXYStartAddrEndAddr(x0, x1-1, y0, y0+7);
    for(i=0; i<(bar<<3); i++) LCD_WD(Scolor);
}
...
#ifdef LCD_320X240_ILI9328
    y1 = LCD_SCREEN_WIDTH - y1;
    y2 = LCD_SCREEN_WIDTH - y2;
#endif
...
void DisplayGB1616_Direction(unsigned char postion_x, unsigned char  postion_y, unsigned char direction, unsigned char Code[2], unsigned int fColor,unsigned int bColor)
{
    ...
    if('D' == direction)
    {
        LCD_WR(0x36);
        LCD_WD(0xE0);
        SetXYStartAddrEndAddr(LCD_SCREEN_HIGH-(postion_x*16+15), LCD_SCREEN_HIGH-postion_x*16, postion_y*16, postion_y*16+15);
    }
    if('L' == direction)
    {
        LCD_WR(0x36);
...
#ifdef LCD_320X240_ILI9328
    //y = (LCD_SCREEN_WIDTH>>4) - (1+y);
    unsigned int postion_y0 = (LCD_SCREEN_WIDTH>>4) - (1+postion_y);
    SetXYStartAddrEndAddr(postion_x*8, postion_x*8+7, postion_y0*16, postion_y0*16+15);
#else
...
void LCD_Show_Line(void)
{
    ...
    for (i=0; i<FINGER_NUM_MAX; i++)
    {
        ...
        X[i]  = (uint16_t)(( ((uint32_t)X[i]) * LCD_XMTR_SCALE )>>16);
        Y[i]  = (uint16_t)(( ((uint32_t)Y[i]) * LCD_RECV_SCALE )>>16);
        
        if(X[i] || Y[i])
        {
            if(LCD_SCREEN_HIGH  <= X[i])
            {   
                X[i] = LCD_SCREEN_HIGH - 1;
            }
            if(SCREEN_WIDTH <= Y[i])
            {
                Y[i] = LCD_SCREEN_WIDTH - 1;
            }
            cnt++;
            if(bdt.DPD[i].Finger_X_LCD || bdt.DPD[i].Finger_Y_LCD)
            {
