...
#define   LCD_ID(index,data)  do{ *(__IO  uint16_t *)(( uint32_t )0x60000000)=index;*(__IO  uint16_t *)(( uint32_t )0x60020000)=data;}while(0);
...
extern void TFT_ShowBar(uint16_t x, uint16_t y, uint16_t bar, uint16_t Scolor);
extern void TFT_ShowChar(unsigned int postion_x,unsigned int postion_y,unsigned char num,unsigned int fColor, unsigned int bColor);
